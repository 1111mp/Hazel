  ...
  public:
    ...
    void BeginSession(const std::string& name, const std::string& filepath = "results.json")
    {
      ...
      if (m_CurrentSession)
      {
        ...
        // profiling output.
        if (Log::GetHazelLogger())
        { // Edge case: BeginSession() might be before Log::Init()
          HZ_HAZEL_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
        }
        InternalEndSession();
      }
      ...
      {
        if (Log::GetHazelLogger()) { // Edge case: BeginSession() might be before Log::Init()
          HZ_HAZEL_ERROR("Instrumentor could not open results file '{0}'.", filepath);
        }
      }
    }
